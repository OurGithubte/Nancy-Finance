  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        <AccountListCard accounts={activeAccounts as any} totalBalance={totalBalance} />
          ...
          <div>
            ...
            <div className="mt-4 space-y-3">
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="font-semibold text-foreground">{formatVND(sumBank)}</span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-slate-300">Tiền mặt ({cashAccounts.length})</span>
                <span className="font-semibold text-foreground">{formatVND(sumCash)}</span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-slate-300">Ví điện tử ({ewalletAccounts.length})</span>
                <span className="font-semibold text-foreground">{formatVND(sumEwallet)}</span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-slate-300">Tiết kiệm có kỳ hạn ({savingsAccounts.length})</span>
                <span className="font-semibold text-foreground">{formatVND(sumSavings)}</span>
              </div>
            </div>
          </div>
        </div>
