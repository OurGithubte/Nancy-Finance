...
import { Coins, TrendingUp, CreditCard, ArrowDownRight } from "lucide-react";
...
export default async function DashboardOverviewPage({
  ...
  const smartInsightsPromise = insightsService.getSmartInsights(userId, {
    netWorthHistory: netWorthHistoryPromise,
    forecast30: forecastSummaryPromise,
  });
  ...
  const [
    ...
    forecastSummaryPromise,
    smartInsightsPromise,
  ]);
  ...
  const mappedCategories = summary.expenseCategories.map((c) => ({
    ...
    amount: c.amount,
    percentage:
      summary.kpiSummary.totalExpense > 0
        ? Math.round((c.amount / summary.kpiSummary.totalExpense) * 100)
        : 0,
    color: c.color,
  }));

  const mappedCashflow = cashflow.map((c) => ({ ...c, monthLabel: c.month }));
  ...
  return (
    <div className="space-y-6">
      <FinancePageHeader title="Tổng quan" subtitle="Cập nhật tình hình tài chính của bạn" />
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        <AccountListCard accounts={summary.accounts as any} totalBalance={summary.kpiSummary.availableCash} />
