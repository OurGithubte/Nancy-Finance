...
export function TransactionTable({
  ...
  const getCategoryIcon = (iconName?: string) => {
    switch (iconName) {
      case "utensils":
        return <Utensils className="h-4 w-4 text-credit" />;
      case "trending-up":
        return <TrendingUp className="h-4 w-4 text-income" />;
      case "car":
        return <Car className="h-4 w-4 text-cyan-400" />;
      case "shopping-bag":
        return <ShoppingBag className="h-4 w-4 text-saving" />;
      case "home":
        return <Home className="h-4 w-4 text-income" />;
      default:
        return <Tag className="h-4 w-4 text-muted" />;
    }
  };
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      case "income":
        return <ArrowDownLeft className="h-3 w-3 text-income" />;
      case "expense":
        return <ArrowUpRight className="h-3 w-3 text-expense" />;
      case "transfer":
        return <RefreshCw className="h-3 w-3 text-credit" />;
      default:
        return null;
    }
  };
  ...
  return (
    <div
      className={cn(
        "rounded-2xl border border-border bg-surface/90 p-5 shadow-sm backdrop-blur",
        className
      )}
    >
      {title && (
              ...
              >
                <td className="py-3.5 pr-3">
                  <div className="flex items-center gap-3">
                    <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-surface-card border border-border-card">
                    ...
                    <div>
                      ...
                      <span className="text-[11px] text-muted flex items-center gap-1 sm:hidden">
                        {getTypeIcon(tx.type)}
                ...
                <td className="py-3.5 px-3 text-slate-300 hidden sm:table-cell">
                  <span className="inline-flex items-center rounded-md bg-surface-card px-2 py-0.5 text-[11px] font-medium text-slate-300">
                    {tx.categoryName}
                  </span>
                </td>
                {showAccount && (
                ...
                <td className="py-3.5 px-3 text-muted whitespace-nowrap">
