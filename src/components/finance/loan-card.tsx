...
export interface LoanListCardProps {
  ...
  onArchive?: (loan: LoanItem) => void;
}

export function LoanListCard({ loans, className, onPayment, onEdit, onArchive }: LoanListCardProps) {
  return (
    <div
      className={cn(
            ...
            >
              ...
              <div className="mt-3">
                ...
                <div className="mt-2 flex flex-wrap items-center justify-end gap-2">
                  ...
                  {onEdit && (
                    <button onClick={() => onEdit(loan)} className="text-[10px] px-2 py-1 rounded-md bg-slate-500/10 text-slate-300 hover:bg-slate-500/20 cursor-pointer">
                      Sửa
                    </button>
                  )}
                  {onArchive && (
                    <button onClick={() => onArchive(loan)} className="text-[10px] px-2 py-1 rounded-md bg-red-500/10 text-red-400 hover:bg-red-500/20 cursor-pointer">
                      Lưu trữ
                    </button>
                  )}
                </div>
