...
const FinanceDialog = dynamic(
  () => import("@/components/finance/finance-dialog").then((m) => m.FinanceDialog),
  { ssr: false }
);
const FinanceDrawer = dynamic(
  () => import("@/components/finance/finance-drawer").then((m) => m.FinanceDrawer),
  { ssr: false }
);
const TransactionForm = dynamic(
  () => import("@/components/finance/transaction-form").then((m) => m.TransactionForm),
  { ssr: false }
);
    ...
    <div className="flex min-h-screen bg-background text-foreground">
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:hidden">
          ...
          />
          <div className="relative z-10 w-full rounded-t-3xl border-t border-border bg-surface p-5 shadow-2xl animate-in slide-in-from-bottom duration-300">
            <div className="flex items-center justify-between border-b border-border pb-3">
              <h2 className="text-base font-bold text-foreground">Menu chức năng</h2>
              <button
                type="button"
                onClick={() => setIsMobileMenuOpen(false)}
